...
import type { VectorSearchResult } from './services/vectorStoreCore'
import { USER_DATA_PATH } from './logger'

type UtilityChild = Worker
...
export class WorkerManager extends EventEmitter {
  ...
  private workerScriptPath(): string {
    // In production, we'll need to bundle this worker script or run it with tsx
    return join(__dirname, 'worker', 'index.ts')
  }
  ...
  private routeFromWorker(msg: unknown): void {
    ...
    if (type === 'indexing-progress') {
      ...
      this.emit('indexing-progress', payload)
      return
    }
  ...
  private async doSpawn(): Promise<void> {
    ...
    appendRagWorkerLog(`Spawning worker process script=${workerPath} userData=${USER_DATA_PATH}`)
    
    const env: Record<string, string> = {
      ...process.env,
      KURUMI_USER_DATA: USER_DATA_PATH,
    } as Record<string, string>

    const child = new Worker(workerPath, {
      env,
      // Use tsx or ts-node registers if running from ts
