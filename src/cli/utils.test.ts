...
describe('Context Instructions Audit', () => {
  ...
  it('1. Test one by one: .kurumi/instructions.md', () => {
    ...
    // findInstructions() prepends a [Priority: ...] label so the LLM knows the source.
    // Verify the raw content is present and the file path is recorded.
    expect(result?.content).toContain('KURUMI_TEST')
    expect(result?.content).toContain('[Priority: Highest - Kurumi Native Instructions]')
  ...
  it('2. Parent directory resolution test', () => {
    ...
    const result = findInstructions()
    expect(result?.content).toContain('PARENT_DIR_TEST')
    expect(result?.loadedFiles[0]).toContain('instructions.md')
  })
  ...
  it('3. Case-insensitive filesystem collision risk (Mocked realpathSync)', () => {
    ...
    // Mock readdirSync to return two case variants — simulating a case-insensitive FS
    vi.spyOn(fs, 'readdirSync').mockReturnValue(['CLAUDE.md', 'claude.md'] as any)

    // existsSync must return false for .kurumi/instructions.md (so only CLAUDE.md variants are tested)
    // and true for the CLAUDE.md path itself
    vi.spyOn(fs, 'existsSync').mockImplementation((p) => {
      const s = p.toString()
      if (s.includes('.kurumi')) return false
      return true
    })
    ...
    // Content should appear, and deduplication by realpathSync should result in only 1 file loaded
    expect(result?.content).toContain('MOCK_CONTENT')
  ...
  it('4. Precedence order: .kurumi > CLAUDE.md', () => {
    ...
    const result = findInstructions()
    // Both files are combined, separated by the ---  divider
    expect(result?.content).toContain('KURUMI_NATIVE')
    expect(result?.content).toContain('CLAUDE_COMPAT')
    expect(result?.content).toContain('---')
    // .kurumi/instructions.md must come first
    expect(result?.loadedFiles[0]).toContain('instructions.md')
